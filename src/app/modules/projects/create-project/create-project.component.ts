...
export class CreateProjectComponent implements OnInit {
  ...
  step:any = 0;
    listForStruct= ['NN High', 'NN Low', 'Brute Force'];
        ...
        forkJoin( this.apiRequest.getProjectDetails(id),this.apiRequest.getDatsetList())
          .subscribe((resp:any) => {
            ...
            if (res && res.detail) {
              ...
              //getSourceData
              // if(resp[2] && resp[2].detail ){
              //   this.sourceList=resp[2].detail;
              //   this.sourceTypeList = resp[2].detail['channels'];
              //   this.SourceChannelResponse.sourceChannel = "GCP Bucket (Default)";
              ...
              if(res.detail.drlJson){
                ...
                // if(drlrule1 && drlrule1.rules && drlrule1.rules.length ){
                //   this.drldata = drlrule1.rules;
                //   // console.log(this.drldata)
                //   this.drldata.forEach(
                //     (rule,i)=>{
                //       rule.when.forEach((line,j) => {
                //         line.conditions.forEach((condition,k) => {
                //             let obj = {
                //               header:condition.header || '',
                //               isDatasetOperator:condition.isDatasetOperator || false,
                //               dataset:condition.dataset||'',
                //               category:condition.category||'',
                //               isBreVeriable:condition.isBreVeriable||false,
                ...
                //             }
                //             condition=obj;
                //             this.drldata[i].when[j].conditions[k]=obj;
                //         });
                //       });
                //     }
                //   )
                ...
                //     }
                //   )
                //   this.rulesDataSource.data=this.drldata;
                //   this.rulesDataSource._updateChangeSubscription();
                // }
              }
            }
          }, error => {
...
/**
   * @name createForm
   * @desc creates the reactive form of user details 
   * @param {}
   * @memberOf Class.CreateEditUserComponent
   */
...
getDatasetHeaderList(){
  ...
  // console.log(this.dataSourceForDatasets.data);
...
onDocumentStructureChange() {
  setTimeout(() => {
    if(this.projectInfoForm.value.documentStructure === 'Structured') {
      ...
      // this.projectInfoForm.addControl('processingEngine', new FormControl('', [Validators.required]));
      // this.projectInfoForm.patchValue({
